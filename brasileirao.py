...
def nome_campeao(dados):
    id = dados['fases']['2700']['classificacao']['grupo']['Único'][0]
    return dados['equipes'][id]['nome-comum']
...
def qtos_libertadores(dados):
    string = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa']
    formatacao = string.split('-')[1]
    return  int(formatacao)
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    for pos in range(numero_de_times):
        melhores.append(times[pos])
    return melhores
...
def classificados_libertadores(dados):
    qtd_liberta = qtos_libertadores(dados)
    return ids_dos_melhor_classificados(dados, qtd_liberta)
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    id = ids_dos_times_de_um_jogo(dados, id_jogo)
    nome1 = nome_do_time(dados, id[0])
    nome2 = nome_do_time(dados, id[1])
    return nome1, nome2
...
def id_do_time(dados,nome_time):
    for chave in dados['equipes'].keys():
        nome_chave = nome_do_time(dados, id_numerica=chave)
        if nome_chave == nome_time:
            return chave
...
def data_de_um_jogo(dados,id_jogo):
    jogos = datas_de_jogo(dados)
    for (data,lista_ids_jogos) in jogos.items():
        if id_jogo in lista_ids_jogos:
            return data
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    for chave in jogos.keys():
        resultado = dados['fases']['2700']['jogos']['id'][chave]['estadio_id']
        if resultado in qtd_jogos:
            qtd_jogos[resultado] += 1
        else:
            qtd_jogos[resultado] = 1
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    lista_ids = []
    equipe = dados['equipes']
    for chave in equipe.keys():
        equipes_id =  equipe[chave]
        if nome_time in equipes_id['nome']:
            lista_ids.append(chave)
        
        if nome_time in equipes_id['nome-comum']:
            lista_ids.append(chave)
        
        if nome_time in equipes_id['nome-slug']:
            lista_ids.append(chave)
        
        if nome_time in equipes_id['sigla']:
            lista_ids.append(chave)
...
def ids_de_jogos_de_um_time(dados,time_id):
    jogo_id = dados['fases']['2700']['jogos']['id']
    participacao = []
    for chave in jogo_id.keys():
        resultado = dados['fases']['2700']['jogos']['id'][chave]
        if time_id in resultado['time1']:
            participacao.append(chave)
        
        if time_id in resultado['time2']:
            participacao.append(chave)
...
def datas_de_jogos_de_um_time(dados,nome_time):
    jogo_id = dados['fases']['2700']['jogos']['id']
    datas = []
    nome_time = id_do_time(dados, nome_time)
    for chave in jogo_id.keys():
        resultado = dados['fases']['2700']['jogos']['id'][chave]
        if nome_time in  resultado['time1']:
            datas.append(resultado['data'])
        
        if nome_time in  resultado['time2']:
            datas.append(resultado['data'])
...
def dicionario_de_gols(dados):
    ...
    for chave in jogos.keys():
        ...
        placar1 = int(resultado['placar1'])
        placar2 = int(resultado['placar2'])

        if time1 not in qtd_gols:
            qtd_gols[time1] = placar1
        else:
            qtd_gols[time1] += placar1
        
        if time2 not in qtd_gols:
            qtd_gols[time2] = placar2
        else:
            qtd_gols[time2] += placar2
...
def time_que_fez_mais_gols(dados):
    ...
    gols = 0
    time = None
    for time_id, i in gols_times.items():
        if i > gols:
            gols = i
            time = time_id
    return time
...
def rebaixados(dados):
     ...
     formatacao = faixa.split('-')
     tabela = dados['fases']['2700']['classificacao']['grupo']['Único']
     serie_b = []
     nro1 = int(formatacao[0]) - 1
     nro2 = int(formatacao[1])
     for pos in range(nro1, nro2):
        id_time = tabela[pos]
        serie_b.append(id_time)
     return serie_b
